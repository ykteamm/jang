<div class="modal-content" id="md-con">
        <style>
            .news-menu-item {
                width: 32.2%;
                padding: 9px 25px;
    ...
    {{-- @if($resime == 2) --}}
        ...
        @if ($nw)
            <div id="newsBodyScrollable" class="modal-body p-0" style="position: relative; margin-top:-2px">
                ...
                <div id="newsScrollable" style="position: relative;margin-top:-5px;overflow:hidden">
                    <div id="readNotificationEvent" onclick="readNotificationEvent({{ $nw->id }})" wire:click="$emit('read', {{ $nw->id }})" class="d-none"></div>
                    <div id="updateNews" onclick="setReaction()" class="d-none"></div>
                    <script>
                        const newsBody = document.querySelector("#newsBodyScrollable")
                        const scroll = document.querySelector("#newsScrollable")
                        newsBody.addEventListener("scroll", event => {
                            let height = scroll.scrollHeight;
                            let scrollHeight = Math.abs(scroll.getBoundingClientRect().y) + window.innerHeight
                            if(scrollHeight >= height) {
                                document.querySelector("#readNotificationEvent").click();
                                document.querySelector("#updateNews").click();
                            }
                        }, { passive: true });            
                    </script>
                    ...
                    <img style="margin-top:-33px" width="100%" src="{{ asset('mobile/news/nwf.png') }}" alt="Img">
                </div>
            </div>
        @else
            <div id="newsBody">
