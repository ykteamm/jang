...
class MakeImageService
{
  ...
  {
    $this->img = imagecreatefromjpeg(public_path('winners/wintemplate.jpg'));
  ...
  private function makeRectLose($w, $h, int $dst_x, int $dst_y, int $src_x, int $src_y, $summa, $isNum)
  {
    $image = imagecreate($w, $h);
    imagecolorallocate($image, 56, 48, 216);
    ...
    $start = ($w - $len * 40) / 2;
    imagettftext($image, 24, 0, $start + 30, 55, $color, $this->ballFont, $summa);
    imagecopymerge($this->img, $image, $dst_x, $dst_y, $src_x, $src_y, $w, $h, 100);
    return $this;
  }

  private function makeRectWin($w, $h, int $dst_x, int $dst_y, int $src_x, int $src_y, $summa, $isNum)
  {
    $image = imagecreate($w, $h);
    imagecolorallocate($image, 188, 5, 5);
    $color = imagecolorallocate($image, 255, 255, 255);
    imagealphablending($image, true);
    $sum = str_split((string)$summa);
    $len = count($sum);
  ...
  public function make($battle)
  {
    ...
    $winner = User::find($battle['win']);
    $loser = User::find($battle['lose']);
    ...
    // ->destroy();
    $this
      ->setImage($winner['image_url'], 660, 0, 0, 0, 130, 130, 100)
      ->makeRectWin(300, 80, 950, 690, 0, 0, strtoupper($winnerName), false)
      ->makeRectWin(300, 80, 950, 760, 0, 0, $sum1, true)
      ->setImage($loser['image_url'], 320, 330, 0, 0, 130, 130, 100)
      ->makeRectLose(300, 80, 630, 690, 0, 0, strtoupper($loserName), false)
      ->makeRectLose(300, 80, 630, 760, 0, 0, $sum2, true)
      ->save($innerPath)
      ->destroy();


    DB::table('user_battles')
      ->where('id', $battle['id'])
